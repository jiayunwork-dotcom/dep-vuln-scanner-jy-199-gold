# dep-vuln-scanner —— 依赖漏洞扫描与 SBOM 生成 CLI
...
# 常用运行方式（在宿主机改、容器里跑）：
#   docker build -t dep-vuln-scanner .
#   docker run --rm -v "$PWD":/src dep-vuln-scanner go run . -manifest /src/example/dependencies.lock -format report
